...
export const HeroTitle = ({ children, className, ...props }: TypographyProps) => {
  return (
    <motion.h1 
      className={cn("text-5xl md:text-7xl lg:text-9xl font-black uppercase tracking-tighter text-white leading-[0.85]", className)}
      {...props as any}
    >
      {children}
    </motion.h1>
...
export const SectionTitle = ({ children, className, ...props }: TypographyProps) => {
  return (
    <motion.h2 
      className={cn("text-3xl md:text-5xl lg:text-6xl font-black uppercase tracking-tighter text-white", className)}
      {...props as any}
    >
      {children}
    </motion.h2>
...
export const ScrollRevealHeading = ({ text, className, ...props }: { text: string; className?: string } & HTMLMotionProps<"h2">) => {
  ...
  return (
    <motion.h2 
      ref={ref}
      className={cn("text-3xl md:text-5xl lg:text-7xl font-black uppercase tracking-tighter text-white flex flex-wrap gap-x-0 md:gap-x-4", className)}
      {...props as any}
    >
      {words.map((word, i) => {
